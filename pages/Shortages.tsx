...
const MOCK_HISTORY = [
    { index: '1.56', esfCil: '+1.00 -2.25', user: 'Admin', treatment: 'Filtro Azul (Verde)', company: 'Master', time: 'Agora', quantity: 1, type: 'Incolor' },
...
const Shortages: React.FC = () => {
  ...
  const typeOptions = ['Incolor', 'Fotossensível (Photo)'];
  const coatingOptions = ['Incolor', 'AR', 'Filtro Azul (Verde)', 'BlueCut (Azul)'];

  const is149 = formData.material === '1.49';

  // Load local mock history
  useEffect(() => {
    if (isHistoryOpen) {
      setRecentHistory(MOCK_HISTORY);
    }
  }, [isHistoryOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // const esfCil = `${formData.sphere} ${formData.cylinder}`;
              ...
              <div className="max-w-5xl mx-auto space-y-6 w-full py-2">
                ...
                <div>
                   ...
                   <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                      <div>
                        <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-1">Índice de Refração</label>
                        <div className="relative">
                          <select 
                            ...
                            required
                            className="w-full appearance-none rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 px-4 py-2.5 text-slate-900 dark:text-white text-sm font-bold focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800"
                          >
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-1">Tipo</label>
                        <div className="relative">
                          <select 
                            name="lensType"
                            ...
                            disabled={is149}
                            required
                            className={`w-full appearance-none rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 px-4 py-2.5 text-slate-900 dark:text-white text-sm font-bold focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all ${is149 ? 'opacity-60 cursor-not-allowed bg-slate-100 dark:bg-slate-800' : 'cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                          >
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-1">Tratamento</label>
                        <div className="relative">
                          <select 
                            name="coating"
                  ...
                  recentHistory.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-4 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-2xl border border-slate-100 dark:border-slate-700">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{item.treatment}</p>
                        </div>
                    </div>
                  ))
              )}
